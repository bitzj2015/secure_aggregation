...
def main(args):
    ...
    np.random.seed(0)
    clientSubSet = np.random.choice(nClients-1, subset-1, replace=False)
    ...
    for iRound in range(trainTotalRounds):
        ...
        local_model_ipdates = ray.get([worker.push_local_model_updates.remote(global_model_param) for worker in workerByClient])
        for name in global_model_param.keys():
            cur_param = torch.cat([item[name].unsqueeze(0) for item in local_model_ipdates], axis=0)
